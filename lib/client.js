...
`;
    ...
    function sendNotification(title, body, sessionId, ctx) {
      if (!("Notification" in window)) { console.warn("[dsh-mini-window] Notification API missing in renderer"); return; }
      if (Notification.permission !== "granted") { console.warn("[dsh-mini-window] permission not granted:", Notification.permission); return; }
      try {
      ...
      ctx.effect(() => {
        ...
        var scan = function () {
          ...
          try {
            ...
            console.warn("[dsh-mini-window] getSnapshot threw:", e);
            return;
          }
          // Render-layer snapshot shape: { ids: SessionId[], byId: Record<id, SessionSummary>, ... }.
          var byId = snapshot && snapshot.byId;
          var ids = (snapshot && snapshot.ids) || (byId ? Object.keys(byId) : []);
          if (!ids || !ids.length) return;

          for (var i = 0; i < ids.length; i++) {
            var id = ids[i];
            var row = (byId && byId[id]) || null;
            if (!row) continue;
        ...
        if (sessions && sessions.list) {
          try {
            ...
            var byId0 = snapshot0 && snapshot0.byId;
            var ids0 = (snapshot0 && snapshot0.ids) || (byId0 ? Object.keys(byId0) : []);
            for (var j = 0; j < ids0.length; j++) {
              var row0 = byId0 && byId0[ids0[j]];
              if (!row0) continue;
              prevRunning.set(ids0[j], Boolean(row0.running));
              prevPending.set(ids0[j], row0.pendingInteraction);
            }
          } catch (e) {
            /* list not ready yet */
          }
