      ...
      ctx.effect(() => {
        ...
        if (sessions && sessions.list) {
          try {
            ...
            if (byId0) {
              for (var id0 in byId0) {
                if (!Object.prototype.hasOwnProperty.call(byId0, id0)) continue;
                var row0 = byId0[id0];
                if (!row0) continue;
                prevRunning.set(id0, Boolean(row0.running));
                prevPending.set(id0, row0.pendingInteraction);
              }
            }
          } catch (e) {
            /* list not ready yet */
