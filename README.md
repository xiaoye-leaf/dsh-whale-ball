...
- **v1.0.7（当前）**
  - **真正修复页面通知从未触发**（v1.0.5 修复方向错误）：页面渲染层 `sessions.list.getSnapshot()` 返回的是 `{ ids: [...], byId: {...} }` 结构，**没有 `items` 字段**——v1.0.5 误把 `byId` 改成 `items`（那是桌面壳 node 端 RPC 的格式），导致页面层一直拿到空列表、任务完成/等待确认时页面通知静默失效。本次改为正确遍历 `byId`（字段名 `id`），实测任务完成通知已正常弹出
  - **新增运行时诊断日志**：桌面壳写入 `%USERPROFILE%\.dsh\dsh-mini-window-diag.log`，记录通知调用、`show`/`failed` 事件与渲染进程 console，通知异常时无需 DevTools 即可排查
  - 页面通知与桌面壳通知的分工说明：页面层通知在主窗口可见时弹出；桌面壳层系统通知（toast）在主窗口最小化/隐藏时弹出
- **v1.0.6**
  - **修复桌面版（Windows）通知静默失败**：Electron 在 Windows 上弹系统通知需要 AppUserModelID（`app.setAppUserModelId`），桌面壳此前未设置导致 toast 悄悄失败——已加上，任务完成/等待确认的通知现在真正可见
  - **启动时自动申请通知权限**：`client.js` 不再依赖先点右下角按钮才请求权限，页面加载即自动请求（Electron 静默授予；浏览器会弹系统询问），通知开箱即用
- **v1.0.5**
  - **页面通知触发的初步修复**：读取会话快照时改用 DSH 实际返回的字段（v1.0.7 中修正为正确的 `byId` 结构），为页面通知真正可用铺路
- **v1.0.4**
